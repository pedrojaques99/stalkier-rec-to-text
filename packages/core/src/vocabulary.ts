 ...
 *
 * Este módulo é PURO: não sabe onde a lista mora. Quem persiste é o app — num
 * arquivo do usuário, no config do engine, no que for. É essa ignorância que
 * deixa o mesmo código servir o app público e o Jaques Studio sem cada um ter
 * a sua cópia da regra.
 */
...
/** Normaliza a lista antes de gravar. Quem grava é o app; a regra é daqui. */
export function normalizeVocabulary(list: unknown): string[] {
  return [...new Set((Array.isArray(list) ? list : []).map((s) => String(s).trim()))]
